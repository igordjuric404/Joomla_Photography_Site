						...
						<?php foreach($this->fields as $fieldName => $oneExtraField){
							...
							<tr id="hikashop_product_<?php echo $fieldName; ?>">
								...
								<td>
									...
									<?php echo $this->fieldsClass->display($oneExtraField,$this->element->$fieldName,'data[product]['.$fieldName.']',false,' '.$onWhat.'="window.hikashop.toggleField(this.value,\''.$fieldName.'\',\'product\',0);"'); ?>
								</td>
							</tr>
							<?php }
						} ?>
...
$html = array();
$app->triggerEvent('onProductBlocksDisplay', array(&$this->element, &$html));
if(!empty($html)){
	foreach($html as $h){
		echo $h;
	}
