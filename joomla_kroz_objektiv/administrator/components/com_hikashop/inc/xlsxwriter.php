...
class XLSXWriter_BuffererWriter
{
	...
	public function __construct($filename, $fd_fopen_flags='w', $check_utf8=false)
	{
		...
		if ($this->fd===false) {
			HikashopXlsxwriterInc::log("Unable to open $filename for writing.");
		}
	}
	...
	protected function purge()
	{
		if ($this->fd) {
			if ($this->check_utf8 && !self::isValidUTF8($this->buffer)) {
				HikashopXlsxwriterInc::log("Error, invalid UTF8 encoding detected.");
