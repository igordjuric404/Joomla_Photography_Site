	...
	public function loadGoogleFont(string $fontName)
	{
		...
		if (!empty($fontName))
		{
			if (!\in_array($fontName, $systemFonts))
			{
				...
				$googleFontUrl = '//fonts.googleapis.com/css?family=' . $fontName . ':100,100italic,200,200italic,300,300italic,400,400italic,500,500italic,600,600italic,700,700italic,800,800italic,900,900italic&display=swap';
				$params = ComponentHelper::getParams('com_sppagebuilder');
				$disableGoogleFonts = $params->get('disable_google_fonts', 0);
				if (empty($disableGoogleFonts))
				{	
					Factory::getDocument()->addStylesheet($googleFontUrl);
				}
			}
		}
	}
