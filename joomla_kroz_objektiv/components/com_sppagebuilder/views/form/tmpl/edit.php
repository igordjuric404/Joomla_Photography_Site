...
$this->item->text = SpPageBuilderAddonHelper::__($this->item->text, true, SpAddonsConfig::$addons);
...
foreach ($addons_list as &$addon) {
	...
	/**
	 * If inline settings found in the addons settings then read the
	 * fields and add the standard values to the default array.
	 */
	if (isset($addon['inline'])) {
		foreach ($addon['inline']['buttons'] as $button) {
			if (isset($button['fieldset'])) {
				if (isset($button['fieldset']['tab_groups'])) {
					...
					$addon['default'] = array_merge(
						$addon['default'],
						SpPgaeBuilderBase::getSettingsDefaultValue($button['fieldset'])['default']
					);
				}
			}
		}
...
<div id="sp-page-builder" class="sp-pagebuilder <?php echo $menuClassPrefix; ?> page-<?php echo $this->item->id; ?>" data-pageid="<?php echo $this->item->id; ?>">
	<form action="<?php echo Route::_('index.php?option=com_sppagebuilder&id=' . (int) $this->item->id); ?>" method="post" name="adminForm" id="adminForm" class="form-validate page-builder-form" style="display: none;">
		...
		$iframeUrl = Uri::root() . 'index.php?option=com_sppagebuilder&view=form&id='. $this->item->id . '&layout=edit-iframe&Itemid=' . $Itemid . $langCodeInUrl;
	...
	<iframe name="sp-pagebuilder-view" id="sp-pagebuilder-view" class="builder-iframe-desktop" data-url="<?php echo $iframeUrl;?>"></iframe>
