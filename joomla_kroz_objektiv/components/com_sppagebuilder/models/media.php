...
class SppagebuilderModelMedia extends ListModel
{
	...
	public function getFolders()
	{
		...
		$input = Factory::getApplication()->input;
		$path = $input->post->get('path', '/' . $file_path, 'PATH');
		...
		$items = Folder::files(JPATH_ROOT . $path, '.png|.jpg|.jpeg|.gif|.svg|.pdf|.webp', false, true);
		...
		$crumbs = explode(DIRECTORY_SEPARATOR, rtrim(ltrim($path, DIRECTORY_SEPARATOR), DIRECTORY_SEPARATOR));
		$count = count($crumbs);
		...
		foreach ($crumbs as $key => $crumb)
		{
			...
			$breadcrumbs[$key]['path'] = $key > 0 ? dirname($path, $count - $key) . '/' . $crumb : '/' . $file_path;
		}
